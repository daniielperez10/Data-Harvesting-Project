...
ggplot(avg_price_per_city, aes(x = reorder(location, avg_price), y = avg_price, fill = avg_price)) +
  ...
  geom_text(aes(label = sprintf("€%.2f", avg_price)), vjust = -0.5, color = "black", size = 3) + 
...
data_madrid_tp <- data_madrid %>%
  group_by(theater_name) %>%
  summarise(avg_price = round(mean(prices, na.rm = TRUE), 2)) # 27 different theaters
```
